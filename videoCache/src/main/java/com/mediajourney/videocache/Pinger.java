...
class Pinger {
    private static final String TAG = "CacheLog-Pinger";
    ...
    boolean ping(int maxAttempts, int startTimeout) {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                if (CacheLog.DEBUG){
                    CacheLog.w(TAG,"Error pinging server (attempt: " + attempts + ", timeout: " + timeout + "). ");
                }
            } catch (InterruptedException | ExecutionException e) {
                if (CacheLog.DEBUG){
                    CacheLog.e(TAG,"Error pinging server due to unexpected error "+ e.getMessage());
                }
            }
            attempts++;
            timeout *= 2;
        ...
        if (CacheLog.DEBUG){
            CacheLog.e(TAG,error+new ProxyCacheException(error));
        }
        return false;
    }
    ...
    private boolean pingServer() throws ProxyCacheException {
        ...
        //持有Url，开启UrlConnection，来获取一个InputStream
        ...
        try {
            ...
            if (CacheLog.DEBUG){
                CacheLog.i(TAG,"Ping response: `" + new String(response) + "`, pinged? " + pingOk);
            }
            return pingOk;
        } catch (ProxyCacheException e) {
            if (CacheLog.DEBUG){
                CacheLog.e(TAG,"Error reading ping response"+ e.getMessage());
            }
            return false;
        } finally {
            source.close();

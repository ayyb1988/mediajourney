...
public class HttpUrlSource implements Source {
    private static final String TAG = "CacheLog-HttpUrlSource";

    private static final int MAX_REDIRECTS = 5;
    //存储sourceinfo信息，默认使用DB存
    ...
    public void open(long offset) throws ProxyCacheException {
        try {
            //这里通过HttpURLConnection,对于小文件而言，HttpURLConnection和OKHttp性能耗时差不多，大文件（20M+）可以采用OKhttp获取更好的性能
    ...
    public void close() throws ProxyCacheException {
        if (connection != null) {
            try {
                ...
                if (CacheLog.DEBUG) {
                    CacheLog.e(TAG, "Error closing connection correctly. Should happen only on Android L. " +
    ...
    private void fetchContentInfo() throws ProxyCacheException {
        if (CacheLog.DEBUG) {
            CacheLog.e(TAG, "Read content info from " + sourceInfo.url);
        }
        ...
        try {
            ...
            this.sourceInfoStorage.put(sourceInfo.url, sourceInfo);
            if (CacheLog.DEBUG) {
                CacheLog.d(TAG, "Source info fetched: " + sourceInfo);
            }
        } catch (IOException e) {
            if (CacheLog.DEBUG) {
                CacheLog.e(TAG, "Error fetching info from " + sourceInfo.url + " "+e.getMessage());
            }
        } finally {
    ...
    private HttpURLConnection openConnection(long offset, int timeout) throws IOException, ProxyCacheException {
        ...
        do {
            if (CacheLog.DEBUG) {
                CacheLog.d(TAG, "Open connection " + (offset > 0 ? " with offset " + offset : "") + " to " + url);
            }

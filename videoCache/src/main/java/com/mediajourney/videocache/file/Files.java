...
/**
 * CacheUtils for work with files.
 *
 * @author Alexey Danilov (danikula@gmail.com).
 */
class Files {

    private static final String TAG = "CacheLog-Files";
        ...
        if (file.exists()) {
            ...
            if (!modified) {
                ...
                if (file.lastModified() < now) {
                    ...
                    if (CacheLog.DEBUG) {
                        CacheLog.w(TAG, "Last modified date {" + new Date(file.lastModified()) + "} is not set for file {" + file.getAbsolutePath() + "}");
                    }
                }
            }
        }
